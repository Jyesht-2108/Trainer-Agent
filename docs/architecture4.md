📊 Evaluation Agent — Architecture Document
...
Retrieve:
•	model["gcs_url"] → trained model path
•	dataset["gcs_url"] → test data path
________________________________________
3️⃣ Download Files from GCP
...
Download model → model.pth
Download dataset → data/test/
________________________________________
4️⃣ Evaluate Model with PyTorch
import torch
...
________________________________________
6️⃣ Send Summary to User
Add to messages table:
supabase.table("messages").insert({
  "user_id": project["user_id"],
  "role": "assistant",
  "content": f"✅ Model evaluation complete! Accuracy: {acc:.2%}"
}).execute()
________________________________________
...
2.	Triggered automatically after pending_training → pending_evaluation.
3.	Updates Supabase → chatbot shows final results.
________________________________________
🧾 Testing Checklist
...
Supabase update	projects.status → completed.
Chat message	Final results visible to user.
________________________________________
✅ Deliverables for Member 4
•	 FastAPI service (/agents/evaluation)
•	 PyTorch model loading & metrics calculation
•	 Supabase update for results and status
